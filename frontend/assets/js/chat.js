...
$(document).ready(function() {
  ...
  function callLexBotWithInitialMessage() {
    // You can send a message that is likely to trigger the GreetingIntent in your Lex bot
    var initialMessage = "Hi"; // or any other message that triggers the GreetingIntent
    callChatbotApi(initialMessage)
      .then((response) => {
        // Handle the response from Lex
        var lexMessage = response.data.messages[0].unstructured.text;
        insertResponseMessage(lexMessage);
      })
      .catch((error) => {
